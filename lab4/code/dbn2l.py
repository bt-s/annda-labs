...
class DeepBeliefNetTwoLayers():
    ...
    def train_greedylayerwise(self, X, y, n_iterations, load_from_file=False,
        ...
        if load_from_file:
            ...
            if compute_rec_err:
                err = self.rbm_stack["hid--pen"].cd1(X, n_iterations=n_iterations,
                        compute_rec_err=compute_rec_err)
                self.reconstruction_errors.append(err)
            else:
                self.rbm_stack["hid--pen"].cd1(X, n_iterations=n_iterations)
            ...
            if compute_rec_err:
                plot_reconstruction_err(self.reconstruction_errors,
                        fname="plots_and_animations/errors_2l.pdf", save_fig=True)
